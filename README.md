...
```yaml
parameters:
  - name: envLabel     # e.g. 'dev', '${{ env }}' (nonprod loop var), 'prod', 'prodfix-01'
    type: string
  - name: artifactName
    type: string
  - name: deployCode
...
steps:
  ...
  - ${{ if eq(parameters.deployCode, true) }}:
    - task: PowerShell@2
      displayName: "Normalize version metadata (${{ parameters.envLabel }})"
      inputs:
        targetType: 'inline'
        script: |
          $versionParts = '${{ parameters.artifactName }}' -split '\-b'
    ...
    - task: DownloadPipelineArtifact@2
      inputs:
        ...
        artifactName: '$(artifactName)'
        itemPattern: '**'
        targetPath: '$(downloadedArtifacts)/${{ parameters.envLabel }}'

  # build the new app settings file we intend to deploy
  - task: PowerShell@2
    ...
    inputs:
      ...
      script: |
        ...
        & "$(Build.SourcesDirectory)/scripts/build-app-settings.ps1" -EnvName '${{ parameters.envLabel }}' -OutputPath '$(appSettingsFile)'
      failOnStderr: true
      showWarnings: true
      pwsh: true

  # deploy code and/or app settings
