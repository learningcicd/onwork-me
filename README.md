...
```yaml
parameters:
  - name: envLabel        # display label used in log headers, e.g. 'dev', 'prod', 'prodfix-01'
    type: string
  - name: scriptEnvName   # value passed to build-app-settings.ps1 -EnvName
    type: string
  - name: azureSubscription
    type: string
  - name: functionAppNameVar   # runtime macro, e.g. '$(functionAppName)' or '$(prodFunctionAppName)'
    type: string
    default: '$(functionAppName)'
  - name: newSettingsFile      # runtime macro, e.g. '$(appSettingsFile)' or '$(prodAppSettingsFile)'
    type: string
    default: '$(appSettingsFile)'
  - name: liveSettingsFile     # must be unique per job so parallel jobs don't collide
    type: string
  - name: artifactName
    type: string
  - name: deployAppSettings
...
steps:
  ...
  - task: AzureCLI@2
    ...
    displayName: "Capture existing app settings (${{ parameters.envLabel }})"
    condition: eq(${{ parameters.deployAppSettings }}, true)
    inputs:
      ...
      inlineScript: |
        $appName = "${{ parameters.functionAppNameVar }}"
        $liveFile = "${{ parameters.liveSettingsFile }}"
  ...
  - task: PowerShell@2
    name: buildAppSettings
    displayName: "Build App Settings (${{ parameters.envLabel }})"
    condition: eq(${{ parameters.deployAppSettings }}, true)
    inputs:
      targetType: 'inline'
      script: |
        New-Item -Path '${{ parameters.newSettingsFile }}' -Value '' -Type File -Force
        & "$(Build.SourcesDirectory)/scripts/build-app-settings.ps1" -EnvName '${{ parameters.scriptEnvName }}' -OutputPath '${{ parameters.newSettingsFile }}'
      failOnStderr: true
      showWarnings: true
      pwsh: true
  ...
  - task: PowerShell@2
    ...
    displayName: "Diff app settings: live vs new (${{ parameters.envLabel }})"
    condition: eq(${{ parameters.deployAppSettings }}, true)
    inputs:
      targetType: 'inline'
      pwsh: true
      script: |
        $liveFile = "${{ parameters.liveSettingsFile }}"
        $newFile  = "${{ parameters.newSettingsFile }}"
        ...
        # Classification, rewritten per review feedback for clarity:
        # 1. ADDED     - only in the new file
        # 2. LIVE-ONLY - only live, not managed by this deploy
        # 3. otherwise it's in both -> split into UNCHANGED / CHANGED
        $rows = foreach ($k in $allKeys) {
          $inLive = $live.ContainsKey($k)
          $inNew  = $new.ContainsKey($k)
          if (-not $inLive -and $inNew) {
            [pscustomobject]@{ Action='ADDED';     Setting=$k; OldValue='';                      NewValue=(Truncate $new[$k] 45) }
          }
          elseif ($inLive -and -not $inNew) {
            [pscustomobject]@{ Action='LIVE-ONLY'; Setting=$k; OldValue='';                      NewValue='' }
          }
          elseif ($inLive -and $inNew) {
            if (ValuesEqual $live[$k] $new[$k]) {
              [pscustomobject]@{ Action='UNCHANGED'; Setting=$k; OldValue='';                     NewValue='' }
            } else {
              [pscustomobject]@{ Action='CHANGED';   Setting=$k; OldValue=(Truncate $live[$k] 45); NewValue=(Truncate $new[$k] 45) }
            }
          }
        }
        ...
        Write-Host ""
        Write-Host "App settings diff for ${{ parameters.envLabel }}  (deploy MERGES: LIVE-ONLY rows are left untouched, nothing is deleted)"
        Write-Host ""
